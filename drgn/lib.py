...
if crash_file == "":
    ...
    prog = drgn.program_from_core_dump(crash_file)
...
def flow_table2(name, table):
    print("\nflow table name: %s\nflow table id: %x leve: %x, type: %x (FS_FT_FDB: %d, FS_FT_NIC_RX: %d, FS_FT_NIC_TX: %d)" % \
        (name, table.id.value_(), table.level.value_(), table.type, prog['FS_FT_FDB'], prog['FS_FT_NIC_RX'], prog['FS_FT_NIC_TX']))
...
def flow_table(name, table):
    print("\nflow table name: %s\nflow table id: 0x%x table_level: %d, \
        ...
        prog['FS_FT_FDB'], prog['FS_FT_NIC_RX'], prog['FS_FT_NIC_TX'], \
        table.max_fte, table.max_fte, \
...
#     print_fs_dr_rule(fte)
    print("fs_fte %lx\tflow_source: %x (0: any, 1: uplink: 2: local), refcount: %d" % \
        (fte.address_of_().value_(), act_dests.flow_context.flow_source, fte.node.refcount.refs.counter))
    val = fte.val
#     print(val)
#     smac = str(ntohl(hex(val[0])))
    print("%8x: " % fte.index.value_(), end='')
    tag = act_dests.flow_context.flow_tag
    if tag:
...
#     metadata_reg_c_0
    ...
    if source_sqn:
        print(" source_sqn: %6x" % (source_sqn >> 8), end='')
    ...
    if source_sqn:
        print(" source_eswitch_owner_vhca_id: %x/%x" % (
            source_eswitch_owner_vhca_id, source_eswitch_owner_vhca_id_mask), end='')
    ...
    print(" action: %4x" % act_dests.action.action.value_())
