...
alias clone-ofed='git clone "ssh://cmi@git-nbu.nvidia.com:12023/mlnx_ofed/mlnx-ofa_kernel-4.0" --branch=mlnx_ofed_24_04;  cp ~cmi/commit-msg mlnx-ofa_kernel-4.0/.git/hooks/'
...
set -x
	...
	$TC filter add dev $rep2 ingress protocol ip chain 0 prio 3 flower $offload \
		dst_mac $mac2 ct_state -trk \
		action sample rate $rate group 5 trunc 60 \
		action ct pipe \
		action goto chain 1
...
# 	devlink dev param set pci/0000:08:00.0 name flow_steering_mode value smfs cmode runtime
	...
	ip xfrm state add src $link_ip dst $link_remote_ip proto esp spi 10001 reqid 100001 \
		...
		sel src $link_ip dst $link_remote_ip
	...
	ip xfrm state add src $link_remote_ip dst $link_ip proto esp spi 10000 reqid 100000 \
		...
		sel src $link_remote_ip dst $link_ip

	ip xfrm policy add src $link_ip dst $link_remote_ip dir out tmpl src $link_ip dst $link_remote_ip proto esp reqid 100001 mode transport
	ip xfrm policy add src $link_remote_ip dst $link_ip dir in tmpl src $link_remote_ip dst $link_ip proto esp reqid 100000 mode transport
	ip xfrm policy add src $link_remote_ip dst $link_ip dir fwd tmpl src $link_remote_ip dst $link_ip proto esp reqid 100000 mode transport
